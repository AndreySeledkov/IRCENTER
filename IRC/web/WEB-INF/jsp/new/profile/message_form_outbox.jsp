    ...
    <div class="list_element_center">
        <ul class="fl_right">
            <li><a href="/messages?section=inbox"
                   onclick="return rightSideUpdateMessagesModal('/messages?section=inbox','inbox',1)">Полученые</a>
            </li>
            <li><a href="/messages?section=outbox"
                   onclick="return rightSideUpdateMessagesModal('/messages?section=outbox','outbox',1)" class="select">Отправленые</a>
            </li>
        </ul>
    </div>
</div>
<div class="profile_right_column_content">
    <c:choose>
        <c:when test="${privateMessagesOutboxCount==0}">
            ПУСТО
        </c:when>
        <c:otherwise>
            <div class="profile_messages_header">
                <span class="messages_counter">Вы получили ${privateMessagesOutboxCount} сообщение</span>
